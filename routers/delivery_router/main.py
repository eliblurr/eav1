...
@router.patch("/{id}/timelines", description="create timeline for delivery", status_code=status.HTTP_201_CREATED, response_model=schemas.DeliveryTimeline)
...
@router.delete("/timelines/{id}", description="remove timeline from delivery", status_code=status.HTTP_201_CREATED)
async def remove_delivery_timeline(id:int, db:Session=Depends(get_db)):
...
@router2.post("/", description="create delivery option", status_code=status.HTTP_201_CREATED, response_model=schemas.DeliveryOption)
...
@router2.get("/", description="read delivery options", response_model=List[schemas.DeliveryOption])
...
@router2.get("/{id}", description="read delivery option by id", response_model=schemas.DeliveryOption)
...
@router2.patch("/{id}", description="update delivery option", response_model=schemas.DeliveryOption, status_code=status.HTTP_202_ACCEPTED)
...
@router2.delete("/{id}", description="delete delivery option", status_code=status.HTTP_202_ACCEPTED)
async def delete_delivery_option(id:int, db:Session=Depends(get_db)):
    return await crud.delete_delivery_option(id, db)

@router2.patch("/{id}/locations", description="add", status_code=status.HTTP_202_ACCEPTED)
...
@router2.delete("/{id}/locations", description="add", status_code=status.HTTP_202_ACCEPTED)
